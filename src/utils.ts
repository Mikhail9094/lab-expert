...
export function printOrDownloadDoc(
  ...
  html2canvas(docRef.current || docRef, {
    ...
    // изменил вывод на печать т.к. в прежнем варианте выводилось новое окно,
    // где нужно было среди разного функционала найти печать в место того,
    //  чтобы сразу попась на страницу печати. С точки зрения пользователя это не удобно.
    print ? printJS({ printable: imgData, type: 'image' }) : pdf.save()
  })
}
